            ...
            for req in risk_mgmt.get('requirements', []):
                if not req.get('compliant', True):
                    findings.append({
                        ...
                        'risk_level': 'high',
                        'severity': 'High',
                        'location': 'Risicobeheer Documentatie / Risk Management Documentation',
                        'ai_act_article': f"Artikel {req.get('sub_article', '9').replace('Art. ', '').replace('Article ', '')}",
            ...
            for req in data_gov.get('requirements', []):
                if not req.get('compliant', True):
                    findings.append({
                        ...
                        'risk_level': 'high',
                        'severity': 'High',
                        'location': 'Data Governance / Training Data Pipeline',
                        'ai_act_article': f"Artikel {req.get('sub_article', '10').replace('Art. ', '').replace('Article ', '')}",
                        'evidence_required': req.get('evidence_required', []),
            ...
            for req in documentation.get('requirements', []):
                if not req.get('compliant', True):
                    findings.append({
                        ...
                        'risk_level': 'medium',
                        'severity': 'Medium',
                        'location': 'Technische Documentatie / Technical Documentation',
                        'ai_act_article': f"Artikel {req.get('sub_article', '11-12').replace('Art. ', '').replace('Article ', '')}",
                        'evidence_required': req.get('evidence_required', []),
            ...
            for req in oversight.get('requirements', []):
                if not req.get('compliant', True):
                    findings.append({
                        ...
                        'risk_level': 'high',
                        'severity': 'High',
                        'location': 'Menselijk Toezicht / Human Oversight Procedures',
                        'ai_act_article': f"Artikel {req.get('sub_article', '14').replace('Art. ', '').replace('Article ', '')}",
                        'evidence_required': req.get('evidence_required', []),
            ...
            for req in accuracy.get('requirements', []):
                if not req.get('compliant', True):
                    findings.append({
                        ...
                        'risk_level': 'high',
                        'severity': 'High',
                        'location': 'Model Kwaliteit / Model Quality & Testing',
                        'ai_act_article': f"Artikel {req.get('sub_article', '15').replace('Art. ', '').replace('Article ', '')}",
                        'evidence_required': req.get('evidence_required', []),
            ...
            for req in fria.get('requirements', []):
                if not req.get('compliant', True):
                    findings.append({
                        ...
                        'risk_level': 'critical',
                        'severity': 'Critical',
                        'location': 'Grondrechten Impact Beoordeling / FRIA Assessment',
                        'ai_act_article': f"Artikel {req.get('sub_article', '29-35').replace('Art. ', '').replace('Article ', '')}",
                        'evidence_required': req.get('evidence_required', []),
            ...
            for req in incident.get('requirements', []):
                if not req.get('compliant', True):
                    findings.append({
                        ...
                        'risk_level': 'high',
                        'severity': 'High',
                        'location': 'Incident Rapportage / Incident Response Procedures',
                        'ai_act_article': f"Artikel {req.get('sub_article', '73').replace('Art. ', '').replace('Article ', '')}",
                        'evidence_required': req.get('evidence_required', []),
            ...
            for req in national_auth.get('requirements', []):
                if not req.get('compliant', True):
                    findings.append({
                        ...
                        'risk_level': 'medium',
                        'severity': 'Medium',
                        'location': 'Autoriteit Naleving / Regulatory Compliance',
                        'ai_act_article': f"Artikel {req.get('sub_article', '70-72').replace('Art. ', '').replace('Article ', '')}",
                        'evidence_required': req.get('evidence_required', []),
