    ...
    try:
        ...
        if '<style id="dgp-preload">' not in content:
            content = content.replace(
                ...
                '          + \'<p id="dgp-retry-msg" style="color:#999;margin-top:24px;font-size:13px"></p>\';\n'
                '        document.addEventListener("DOMContentLoaded", function() {\n'
                '          document.body.appendChild(fb);\n'
                '          var t = setTimeout(function() {\n'
                '            if (!document.querySelector("[data-testid=\\"stApp\\"]") ||\n'
                '                document.querySelector("[data-testid=\\"stApp\\"]").children.length < 2) {\n'
                '              fb.style.display = "flex";\n'
                '              document.getElementById("dgp-retry-msg").textContent =\n'
                ...
                '              clearTimeout(t); clearInterval(check);\n'
                '            }\n'
                '          }, 1000);\n'
                '        });\n'
                '      })();\n'
